...
function buildAndOpen(demande, mode) {
  ...
  const html = [
    "<!DOCTYPE html>",
    "<html lang='fr'><head><meta charset='UTF-8'/><title>Demande INAPI</title>",
    "<style>",
    ...
    "</style></head><body>",
    "<div class='toolbar'><span style='font-size:14px;font-weight:700'>Formulaire INAPI - R2-FO-03</span>",
    "<button onclick='window.print()'>Imprimer / Enregistrer PDF</button></div>",
    "<div class='page'>",
    "<div class='ref'>R2-FO-03<br/>E1</div>",
    "<table class='hdr'><tr>",
    "<td class='hl'><div class='arabic'>المعهد الوطني الجزائري للملكية الصناعية</div><div class='inst'>INSTITUT NATIONAL ALGERIEN</div><div class='inst'>DE LA PROPRIETE INDUSTRIELLE</div></td>",
    "<td class='hm'><div class='logo'><span class='logoi'>in</span>pi</div></td>",
    "<td class='hr'><div class='arabic'>الجمهورية الجزائرية الديمقراطية الشعبية</div><div class='inst'>REPUBLIQUE ALGERIENNE</div><div class='inst'>DEMOCRATIQUE ET POPULAIRE</div></td>",
    "</tr></table>",
    "<div class='nat'><div class='nat-t'>Nature de la demande de protection *</div>",
    ...
    "</tr></table></div>",
    "<div class='fb'><div class='ft'>[71] - DEPOSANT(S)</div><div class='fv' style='min-height:58px'>" + dep1 + dep2 + dep3 + "</div><div class='ff'>Nationalite : " + depNat + "</div></div>",
    ...
    "<div class='pb'><div class='ft'>[30] - REVENDICATION DE PRIORITE</div>",
    "<table class='pt'><thead><tr><th>[31] N depot</th><th>[32] date</th><th>[33] pays</th><th>Nature</th></tr></thead>",
    "<tbody><tr><td>" + priNum + "</td><td>" + priDate + "</td><td>" + paysOri + "</td><td>" + priNat + "</td></tr></tbody></table></div>",
    "<div class='bot'><div class='botl'>",
    "<table class='dt'><thead><tr><th>Numero de depot</th><th>Date de depot</th><th>Heure</th></tr></thead>",
    "<tbody><tr><td>" + numDepo + "</td><td>" + dateDepo + "</td><td>&nbsp;</td></tr></tbody></table>",
    "<div class='di'>N de la demande internationale</div></div>",
    "<div class='vis'><div class='visl'>Visa</div></div></div>",
    "</div>",
    "<div class='page'>",
    "<div class='cert'>Certificat d'addition - brevet principal n : <span class='ul'>" + bretNum + "</span> du <span class='ul'>" + bretDate + "</span></div>",
    ...
    "<table class='rt'><tr><td style='width:30%'>Le prepose a la reception</td><td style='width:35%'>Fait a : le :</td><td style='width:35%;font-style:italic;font-size:8.5pt'>Signature et cachet</td></tr></table>",
    "<div class='ab'><div class='al'>Autres informations</div><div class='av'>" + autresInfo + "</div></div>",
    "<div class='brd'><div class='brt'>BORDEREAU DES PIECES DEPOSEES *</div><div class='brc'>",
    "<div>",
    "<p>" + pCI + " Copie de la demande internationale</p>",
    "<p>" + pMN + " Memoire descriptif langue nationale</p>",
    "<p>" + pMF + " Memoire descriptif original francais</p>",
    "<p>" + pMFD + " Memoire descriptif duplicata francais</p>",
    "<p>" + pDO + " Dessin(s) original(aux)</p>",
    "<p>" + pDD + " Dessin(s) duplicata(aux)</p>",
    "</div><div>",
    "<p>" + pAB + " Abrege descriptif</p>",
    "<p>" + pPO + " Pouvoir</p>",
    "<p>" + pPR + " Document de priorite</p>",
    "<p>" + pCS + " Cession de priorite</p>",
    "<p>" + pTI + " Titre ou justification paiement taxes</p>",
    "</div></div></div>",
    "<div class='coo'>INAPI - 42, rue Larbi BEN MHIDI, Alger - Tel : (021) 73 55 74</div>",
...
export default function RespDemandes() {
  ...
  const [allDemandes, setAll]         = useState([]);
  ...
  const load = async () => {
    try {
      ...
      const enriched = res.map((d) => ({ ...d, _formData: lsGet(d.id_demande) || {} }));
      setAll(enriched);
    } catch {
  ...
  const mesDemandes   = allDemandes.filter((d) => Number(d.createur_id) === currentUserId);
  const agentDemandes = allDemandes.filter((d) => Number(d.createur_id) !== currentUserId);

  const filteredMes = mesDemandes.filter((d) => {
    const fd    = lsGet(d.id_demande) || {};
    const terms = [d.titre, d.nature, d.statut, d.mandataire, fd.deposant_nom, fd.deposant_prenom, fd.inventeur_nom, fd.inventeur_prenom];
    return terms.some((v) => (v || "").toLowerCase().includes(searchMes.toLowerCase()));
  });

  const agentDocRows = agentDemandes.flatMap((d) => {
    const docs = (d.documents || []).filter(
      (doc) => (doc.type_document || "").toLowerCase() === "demande"
    );
    return docs.map((doc) => ({
      demande_id:   d.id_demande,
      statut:       d.statut,
      agent:        d.createur_username || "—",
      brevet:       doc.brevet_info || null,
      nom_document: doc.nom_document,
      date_ajout:   doc.date_ajout,
      fichier_url:  doc.fichier_url || null,
      fichier_nom:  doc.fichier_nom || doc.nom_document,
    }));
  }).filter((row) => {
    const q           = searchAgent.toLowerCase();
    const agentMatch  = row.agent.toLowerCase().includes(q);
    const docMatch    = row.nom_document.toLowerCase().includes(q);
    const brevetTitre = row.brevet ? row.brevet.titre.toLowerCase() : "";
    const brevetNum   = row.brevet ? String(row.brevet.num_brevet).toLowerCase() : "";
    return agentMatch || docMatch || brevetTitre.includes(q) || brevetNum.includes(q);
  });
  ...
  const getNature = () => {
    if (form.nature_brevet)     return "Brevet d'invention";
    if (form.nature_pct)        return "Extension PCT";
    if (form.nature_certificat) return "Certificat d'addition";
    return "";
  };
  ...
  const badgeCls = (s) => {
    if (s === "valider")     return "badge green";
    if (s === "non_valider") return "badge red";
    return "badge orange";
  };

  if (loading) return <p>Chargement...</p>;

  const renderRowMes = (d) => {
    const fd        = lsGet(d.id_demande) || d._formData || {};
    const deposant  = [fd.deposant_nom,  fd.deposant_prenom ].filter(Boolean).join(" ") || "—";
    const inventeur = [fd.inventeur_nom, fd.inventeur_prenom].filter(Boolean).join(" ") || "—";
    return (
      <tr key={d.id_demande}>
        <td>{d.date_depo}</td>
        ...
        <td className="dem-actions">
          ...
          <button className="act-btn dl" title="Telecharger" onClick={() => buildAndOpen(d, "download")}>
  ...
  const renderRowAgent = (row, i) => {
    const brevetLabel = row.brevet
      ? row.brevet.titre + " " + row.brevet.num_brevet
      : null;

    const lienFichier = row.fichier_url
      ? React.createElement(
          "a",
          {
          ...
          React.createElement(InsertDriveFileOutlinedIcon, { style: { fontSize: 15 } }),
          React.createElement("span", null, row.fichier_nom)
        )
      : React.createElement("span", { style: { color: "#aaa", fontSize: 12 } }, "Pas de fichier");

    return (
      <tr key={i}>
        <td>
          <span className="agent-tag">{row.agent}</span>
        </td>
        <td>
          {brevetLabel
            ? <span style={{ fontSize: 13 }}>{brevetLabel}</span>
            : <span style={{ color: "#aaa" }}>—</span>
          }
        </td>
        <td>
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <span style={{ fontWeight: 600, fontSize: 13 }}>{row.nom_document}</span>
            <span style={{ fontSize: 11, color: "#888" }}>{row.date_ajout}</span>
          </div>
        </td>
        <td>{lienFichier}</td>
        <td>
          <span className={badgeCls(row.statut)}>{row.statut}</span>
        </td>
        <td className="dem-actions">
          {row.statut !== "valider" && (
            <button
              className="act-btn valider"
              title="Valider"
              onClick={() => handleValider(row.demande_id)}
            >
              <CheckCircleIcon sx={{ fontSize: 17 }} />
            </button>
          )}
          {row.statut !== "non_valider" && (
            <button
              className="act-btn refuser"
              title="Refuser"
              onClick={() => handleRefuser(row.demande_id)}
            >
    ...
    <>
      <div className="dem-page">
        ...
        <div className="dem-section-header">
          <div className="dem-section-label">
            <span className="section-dot blue" /> Mes demandes
          </div>
        </div>
          ...
          <div className="dem-toolbar">
            <div className="dem-search-wrap">
              ...
              <input
                ...
                placeholder="Rechercher titre, deposant, inventeur..."
                value={searchMes}
            ...
            <table className="dem-table">
              ...
              <tbody>
                {filteredMes.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="dem-empty">Aucune demande enregistree</td>
                  </tr>
                ) : (
                  filteredMes.map((d) => renderRowMes(d))
                )}
              </tbody>
            </table>
          </div>
        ...
        {/* DOCUMENTS AGENTS TYPE DEMANDE */}
        <div className="dem-section-header" style={{ marginTop: 32 }}>
          <div className="dem-section-label">
            <span className="section-dot orange" /> Documents des agents — Type Demande
          </div>
        </div>
        ...
        <div className="dem-card">
          <div className="dem-toolbar">
            <div className="dem-search-wrap">
              ...
              <input
                className="dem-search"
                placeholder="Rechercher agent, brevet, document..."
                value={searchAgent}
                onChange={(e) => setSearchAgent(e.target.value)}
              />
            </div>
            <span className="dem-count">{agentDocRows.length} document(s)</span>
          </div>
          <div className="dem-table-wrap">
            <table className="dem-table">
              <thead>
              ...
              <tbody>
                {agentDocRows.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="dem-empty">
                      Aucun document de type Demande trouve
                    </td>
                  </tr>
                ) : (
                  agentDocRows.map((row, i) => renderRowAgent(row, i))
                )}
              </tbody>
            </table>
          </div>
      ...
      {showModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target.classList.contains("dem-overlay")) {
              setShowModal(false);
            }
          }}
        >
          <div className="dem-modal">
            ...
            <div className="modal-body">
              ...
              <Sec num="📁" label="Informations de depot">
                <div className="modal-grid">
                  <F label="Numero de depot *" name="num_depo"  value={form.num_depo}  onChange={setField} type="number" />
                  <F label="Date de depot *"   name="date_depo" value={form.date_depo} onChange={setField} type="date" />
                </div>
              </Sec>

              <Sec num="71" label="[71] DEPOSANT(S)">
                <div className="modal-grid">
                  <F label="Nom"              name="deposant_nom"          value={form.deposant_nom}          onChange={setField} />
                  <F label="Prenom"           name="deposant_prenom"       value={form.deposant_prenom}       onChange={setField} />
                  <F label="Denomination"     name="deposant_denomination" value={form.deposant_denomination} onChange={setField} full />
                  <F label="Adresse complete" name="deposant_adresse"      value={form.deposant_adresse}      onChange={setField} full area />
                  <F label="Nationalite"      name="deposant_nationalite"  value={form.deposant_nationalite}  onChange={setField} />
                </div>
              </Sec>
              ...
              <Sec num="72" label="[72] INVENTEUR(S)">
                <div className="modal-grid">
                  ...
                  <F label="Prenom"  name="inventeur_prenom"  value={form.inventeur_prenom}  onChange={setField} />
              ...
              <Sec num="30" label="[30] REVENDICATION DE PRIORITE">
                <div className="modal-grid">
                  <F label="N de depot" name="priorite_num_depot" value={form.priorite_num_depot} onChange={setField} />
              ...
              <Sec num="74" label="[74] MANDATAIRE">
                <div className="modal-grid">
                  ...
                  <F label="Prenom"          name="mandataire_prenom"       value={form.mandataire_prenom}       onChange={setField} />
              ...
              <Sec num="i" label="Autres informations">
                <div className="modal-grid">
                  <F label="Informations complementaires" name="autres_informations" value={form.autres_informations} onChange={setField} full area rows={3} />
                </div>
              </Sec>

              <Sec num="📎" label="Bordereau des pieces deposees *">
                <div className="pieces-grid">
                  {[
                    ["piece_copie_int",      "Copie de la demande internationale"],
                    ["piece_memoire_nat",    "Memoire descriptif en langue nationale"],
                    ["piece_memoire_fr",     "Memoire descriptif original (francais)"],
                    ["piece_memoire_fr_dup", "Memoire descriptif duplicata (francais)"],
                    ["piece_dessins_orig",   "Dessin(s) original(aux)"],
                    ["piece_dessins_dup",    "Dessin(s) duplicata(aux)"],
                    ["piece_abrege",         "Abrege descriptif"],
                    ["piece_pouvoir",        "Pouvoir"],
                    ["piece_priorite",       "Document de priorite"],
                    ["piece_cession",        "Cession de priorite"],
...
function F({ label, name, value, onChange, type, full, area, rows }) {
  const t = type || "text";
  const r = rows || 2;
  return (
    <div className={full ? "fg full" : "fg"}>
      <label>{label}</label>
      {area ? (
        <textarea name={name} value={value} onChange={onChange} rows={r} />
      ) : (
        <input type={t} name={name} value={value} onChange={onChange} />
      )}
    </div>
  );
}
